...
def get_courses_grouped_json():
    files = list(os.walk("../assets/courses-qr-codes"))[0][2]
    courses = sorted([i.split(".")[0] for i in files])
    courses_grouped = defaultdict(list)
    [courses_grouped[i[0]].append(i) for i in courses]
    with open("../assets/courses_grouped.json", "w", encoding="utf-8") as f:
        json.dump(courses_grouped, f, ensure_ascii=False, indent=4)
    print("脚本执行完毕")
